...
const pageStyle = `
  ...
  .mt-hline {
    ...
    margin-top: 71px; /* center on avatar */
  }
  ...
  .mt-name {
    ...
    color:#fff; margin-bottom: 2px; min-height: 30px;
    display:flex; align-items:center; justify-content:center;
  }
  .mt-role {
  ...
  /* The mobile-only wrappers are display:none on desktop, shown on mobile */
  ...
  @media(max-width:768px) {
    ...
    /* ── fivozo card on mobile: slightly bigger, full width ── */
    .mob-owner-card {
      margin-bottom: 8px;
    }
    .mob-owner-card .mt-card {
      width: 100%; flex-shrink: 1;
      padding: 32px 24px 26px;
      min-height: auto;
    }
    .mob-owner-card .mt-avatar { width: 96px; height: 96px; }
    .mob-owner-card .mt-name { font-size: 1.35rem; }
    .mob-owner-card .mt-role { min-height: auto; }

    /* ── co-owners: 2-col grid ── */
    .mob-co-grid {
      display: grid;
    ...
    .mob-co-grid .mt-avatar { width: 66px; height: 66px; margin-bottom: 12px; }
    ...
    .mob-crew-grid > :nth-child(even) { transform: translateY(16px); }
    .mob-crew-grid .mt-card {
      width: 100%; flex-shrink: 1;
      padding: 22px 14px 18px;
      min-height: 220px;
    }
    .mob-crew-grid .mt-avatar { width: 66px; height: 66px; margin-bottom: 12px; }
    .mob-crew-grid .mt-placeholder {
      width: 100%; min-height: 160px;
    }
    .mob-crew-grid .mt-placeholder-circle { width: 48px; height: 48px; }
  ...
  @media(max-width:480px) {
    .mob-owner-card .mt-card { padding: 26px 18px 22px; }
    .mob-owner-card .mt-avatar { width: 80px; height: 80px; }
    .mob-owner-card .mt-name { font-size: 1.2rem; }

    .mob-co-grid { gap: 10px; }
    .mob-co-grid .mt-card, .mob-crew-grid .mt-card {
      padding: 18px 10px 16px; min-height: 210px; border-radius: 16px;
    }
    .mob-co-grid .mt-avatar, .mob-crew-grid .mt-avatar { width: 56px; height: 56px; margin-bottom: 10px; }
    .mob-crew-grid { gap: 10px; }
    .mob-crew-grid > :nth-child(even) { transform: translateY(12px); }
    .mob-crew-grid .mt-placeholder { min-height: 140px; border-radius: 16px; }
  ...
  @media(max-width:360px) {
    .mob-owner-card .mt-avatar { width: 68px; height: 68px; }
    .mob-owner-card .mt-name { font-size: 1.1rem; }
    .mob-co-grid .mt-card, .mob-crew-grid .mt-card { padding: 16px 8px 14px; }
    .mob-co-grid .mt-avatar, .mob-crew-grid .mt-avatar { width: 48px; height: 48px; }
...
function MemberCard({ member, featured = false }) {
  ...
  return (
    <motion.div
      className={`mt-card${isOwner ? ' is-owner' : ''}`}
      style={featured ? {
      ...
      <section className="section-pad" style={{ paddingTop: 0 }}>
        <div className="container">
          ...
          <div className="desktop-layout">
            {/* ROW 1 – Wizzard — fivozo (featured) — Mr.Floppa */}
          ...
          {/* ╔══════════════════════════════╗
             ║  MOBILE LAYOUT (new grid)    ║
             ╚══════════════════════════════╝ */}
          <div className="mobile-layout">
            {/* fivozo – full width, slightly bigger */}
            <div className="mob-owner-card">
              {fivozo && <MemberCard member={fivozo} featured />}
            </div>
            ...
            {/* crew staggered 2-col */}
            <div className="mob-crew-grid">
              {supporters.map(m => <MemberCard key={m.id} member={m} />)}
              {placeholders.map(i => <PlaceholderCard key={`mph${i}`} />)}
            </div>
          </div>
